var ClientObjects = {
    Silo: ClientItem.extend({
        ...
        init: function (parent, tileX, tileY) {
            ...
            this.isometric(true)
				.mount(parent)
				.size3d(1 * parent._tileWidth, 1 * parent._tileHeight, parent._tileHeight * 2)
            ...
            this.imageEntity = new IgeEntity()
				.texture(ige.client.gameTexture.silo)
				.dimensionsFromCell()
            //.scaleTo(0.45, 0.45, 1)
                .translateTo(0, -20, 0)
				.drawBounds(false)
    ...
    Barn: ClientItem.extend({
        ...
        init: function (parent, tileX, tileY) {
            ...
            this.isometric(true)
				.mount(parent)
				.size3d(2 * parent._tileWidth, 2 * parent._tileHeight, parent._tileHeight * 2.75)
				.translateToTile(tileX, tileY, 0)
				.mouseOver(function () { this.drawBounds(true); this.drawBoundsData(true); })
				.mouseOut(function () { this.drawBounds(false); this.drawBoundsData(false); })
				.drawBounds(false)
            ...
            // Create the "image" entity
            //this.imageEntity = new IgeEntity()
            //	.texture(ige.client.gameTexture.barn)
            //	.dimensionsFromCell()
            //	.scaleTo(0.45, 0.45, 1)
            //	.drawBounds(false)
            //	.drawBoundsData(false)
    ...
    Coldstorage: ClientItem.extend({
        classId: 'Coldstorage',

        init: function (parent, tileX, tileY) {
            ClientItem.prototype.init.call(this, tileX, tileY, 2, 3);
            var self = this;

            // Setup the 3d bounds container (this)
            this.isometric(true)
				.mount(parent)
				.size3d(2 * parent._tileWidth, 3 * parent._tileHeight, parent._tileHeight * 1)
				.translateToTile(tileX, tileY, 0)
				.mouseOver(function () { this.drawBounds(true); this.drawBoundsData(true); })
				.mouseOut(function () { this.drawBounds(false); this.drawBoundsData(false); })
				.drawBounds(false)
            ...
            // Create the "image" entity
            this.imageEntity = new IgeEntity()
				.texture(ige.client.gameTexture.coldstorage)
				.dimensionsFromCell()
				.scaleTo(0.40, 0.35, 1)
            //.anchor(-5, -20)
				.translateTo(-10, -10, 0)
    ...
    Tomato: ClientItem.extend({
        ...
        init: function (parent, tileX, tileY) {
            ...
            this.isometric(true)
				.mount(parent)
				.size3d(1 * parent._tileWidth, 1 * parent._tileHeight, parent._tileHeight * 1)
				.translateToTile(tileX, tileY, 0)
				.mouseOver(function () { this.drawBounds(true); this.drawBoundsData(true); })
				.mouseOut(function () { this.drawBounds(false); this.drawBoundsData(false); })
				.drawBounds(false)
            ...
            // Create the "image" entity
            this.imageEntity = new IgeEntity()
				.texture(ige.client.gameTexture.seeds_tomato)
				.dimensionsFromCell()
				.scaleTo(0.50, 0.50, 1)
				.anchor(-2, 8)
				.translateTo(0, 0, 0)
    ...
    Corn: ClientItem.extend({
        ...
        init: function (parent, tileX, tileY) {
            ...
            // Create the "image" entity
            this.imageEntity = new IgeEntity()
				.texture(ige.client.gameTexture.seeds_corn)
				.dimensionsFromCell()
				//.scaleTo(0.45, 0.45, 1)
    //        //.anchor(-5, -20)
				.translateTo(8, -10, 0)
    ...
    Wheat: ClientItem.extend({
        ...
        init: function (parent, tileX, tileY) {
            ...
            // Create the "image" entity
            this.imageEntity = new IgeEntity()
				.texture(ige.client.gameTexture.seeds_wheat)
				.dimensionsFromCell()
				//.scaleTo(0.45, 0.45, 1)
            //.anchor(-5, -20)
				//.translateTo(0, 0, 0)
